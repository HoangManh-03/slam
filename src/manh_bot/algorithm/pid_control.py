...
class RobotController(Node):
    ...
    def robot_controller_callback(self):
        ...
        if self.get_clock().now() - self.start_time > Duration(seconds=DELAY):
            ...
            left_distance = self.get_distance(self.depth_image, (0.1, 0.4))  # Vùng bên trái
            right_distance = self.get_distance(self.depth_image, (0.6, 0.9))  # Vùng bên phải
            front_distance = self.get_distance(self.depth_image, (0.4, 0.6))  # Trước mặt
            ...
            if self.turning:
                ...
                if front_distance > 0.3 and abs(cte) < 0.05 :
                    LIN_VEL = 0.3
                    ANG_VEL = 0.0
                    ...
                    LIN_VEL = self.pid_lon.control(min(3.5, front_distance), tstamp)
                    ANG_VEL = -0.15  # Điều chỉnh nhẹ sang phải
                elif left_distance > self.safe_distance and front_distance > 0.3:  # Nếu quá xa tường
                    LIN_VEL = self.pid_lon.control(min(3.5, front_distance), tstamp)
                    ANG_VEL = 0.15  # Điều chỉnh nhẹ sang trái
