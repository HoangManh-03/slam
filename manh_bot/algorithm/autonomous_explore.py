...
def calculate_centroid(x_coords, y_coords):
    ...
    return centroid
#Hàm này chọn nhóm lớn nhất gồm 5 điểm xa hơn target_error*2 và gần robot nhất.
...
def findClosestGroup(matrix,groups, current,resolution,originX,originY):
    ...
    else: #chọn một điểm ngẫu nhiên làm mục tiêu nếu các nhóm gần nhau hơn khoảng cách target_error*2. Điều này cho phép robot thoát khỏi một số tình huống.
...
def exploration(data,width,height,resolution,column,row,originX,originY):
        global pathGlobal #Global path
        data = costmap(data,width,height,resolution)#Mở rộng costmap
        data[row][column] = 0 #Robot Vị trí hiện tại
        data[data > 5] = 1 # 0 là một nơi để đi, 100 là một trở ngại chắc chắn
        data = frontierB(data) #Tìm điểm biên giới
        data,groups = assign_groups(data) #Điểm biên giới nhóm
        groups = fGroups(groups) #Sắp xếp các nhóm từ nhỏ đến lớn. Lấy 5 nhóm hàng đầu
        if len(groups) == 0: #Nếu không có nhóm, khám phá đã hoàn tất
            path = -1
        else: #Tìm nhóm gần nhất nếu có
            data[data < 0] = 1 #-0,05 là vị trí không xác định. Đánh dấu là không có sẵn. 0 = có thể truy cập, 1 = không thể truy cập.
            path = findClosestGroup(data,groups,(row,column),resolution,originX,originY) #Tìm nhóm gần nhất
            if path != None: #Nếu có đường dẫn, hãy sửa nó bằng BSpline
...
class navigationControl(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Image,'/camera/depth/image_raw',self.image_callback,10)
        self.publisher = self.create_publisher(Twist, 'cmd_vel', 10)
        print("[INFORMATION] EXPLORATION MODE ACTIVE")
        self.discovery = True
        self.depth_image = None
        threading.Thread(target=self.exp).start() #Runs the discovery function as a thread.
        
    def exp(self):
        twist = Twist()
        while True: #Chờ cho đến khi dữ liệu cảm biến được gửi đến.
            ...
            if self.discovery == True:
                ...
                self.i = 0
                print("[INFORMATION] NEW TARGET SET")
                t = pathLength(self.path)/speed
                t = t - 0.2 #0,2 giây được trừ đi khỏi thời gian tính theo công thức x = v * t. Sau thời gian t, hàm khám phá được chạy.
                self.t = threading.Timer(t,self.target_callback) #Kích hoạt chức năng trinh sát ngay trước mục tiêu.
                self.t.start()

            else:
                v , w = self.localControl(self.image)
                if v == None:
                ...
                if(abs(self.x - self.path[-1][0]) < target_error and abs(self.y - self.path[-1][1]) < target_error):
                    ...
                    w = 0.0
                    self.discovery = True
                    print("[INFORMATION] GOAL REACHED")
                    self.t.join() #Chờ cho đến khi Thread hoàn tất.
                twist.linear.x = v
                twist.angular.z = w
    ...
    def localControl(self):
        ...
        if front_distance < robot_r:
            if left_distance > right_distance:
                v = 0
                w = math.pi/4  # Quay trái nếu tường gần bên phải hơn
            else:
                v = 0
                w = -math.pi/4 # Quay phải nếu tường gần bên trái hơn
           
        if v == None:
            if front_distance < robot_r or right_distance < 0.6:
                v = 0.2
                2 = -math.pi/4
